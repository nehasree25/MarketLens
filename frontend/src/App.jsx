...
function DashboardRoute({ theme, setTheme, navigate, searchQuery, onSearch }) {
  ...
  if (error) return <main className="message-shell"><div className="message-card"><span className="message-icon">!</span><h1>Dashboard unavailable</h1><p>{error}</p><button className="primary-button" type="button" onClick={() => { setError(null); setData(null); setRetryKey((key) => key + 1) }}>Retry <span>↻</span></button></div></main>
  return <AppLayout path="/dashboard" user={user} theme={theme} searchQuery={searchQuery} onSearch={onSearch} onToggleTheme={() => setTheme(theme === 'dark' ? 'light' : 'dark')} onNavigate={navigate} onLogout={() => { logout(); navigate('/login') }}><Dashboard data={data} searchQuery={searchQuery} /></AppLayout>
}
